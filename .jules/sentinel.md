 ...
 **Learning:** When replacing String.prototype.replace() with RegExp.prototype.exec() in a while loop to manually construct DOM nodes, you must explicitly account for non-global regular expressions. If a RegExp lacks the global ('g') flag, exec() will return the exact same match repeatedly without advancing lastIndex, resulting in an infinite loop that freezes the browser.
 **Prevention:** When using RegExp.prototype.exec() in a loop, always explicitly check for the global flag (e.g., if (!regex.global) break;) or manually advance the loop to prevent infinite loops.
