...
## 2024-06-13 - Dynamic Utility Output Accessibility
**Learning:** Screen readers fail to announce dynamic text updates (such as calculated travel times or rolled encounter results) when they are injected into basic `<div>` containers.
**Action:** When implementing utility output containers that receive dynamic text updates via JavaScript, always add `aria-live="polite"` and `aria-atomic="true"` to ensure screen readers announce the changes cleanly.
