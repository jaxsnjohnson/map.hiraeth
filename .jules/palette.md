...
## 2024-06-14 - Add aria-live to utility output containers
**Learning:** For utility outputs that receive dynamic content (like travel time or encounter results), users relying on screen readers need the updates announced automatically.
**Action:** When creating or modifying dynamic output containers, always add `aria-live="polite"` and `aria-atomic="true"` so that the updated text is immediately accessible without requiring manual refocusing.
